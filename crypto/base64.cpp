...
void Base64::Base64Encoding(istream& is, ostream& os, char makeupChar, const char *alphabet)
{
    int out[4];
    int remain = 0;
    while (!is.eof())
...
//Base64Decoding
void Base64::Base64Decoding(istream& is, ostream& os, char plus, char slash)
{
    int out[3];
    int byte[4];
    int bTmp;
